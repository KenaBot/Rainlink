import { RainlinkEvents, RainlinkLoopMode, RainlinkPlayerState, VoiceConnectState } from "../Interface/Constants.js";
import type { RainlinkSearchOptions, RainlinkSearchResult } from "../Interface/Manager.js";
import type { PlayOptions } from "../Interface/Player.js";
import type { UpdatePlayerInfo, UpdatePlayerOptions } from "../Interface/Rest.js";
import type { RainlinkNode } from "../Node/RainlinkNode.js";
import type { Rainlink } from "../Rainlink.js";
...
import type { RainlinkVoice } from "./RainlinkVoice.js";
...
export class RainlinkPlayer {
	 ...
	 */
	public readonly data: RainlinkDatabase;
	/**
	...
	constructor(manager: Rainlink, voice: RainlinkVoice, node: RainlinkNode) {
		...
		const customQueue = rainlinkOptions.structures?.queue;
		...
		if (rainlinkOptions.structures?.filter) this.filter = new rainlinkOptions.structures.filter(this);
		...
		if (this.voice.options.volume && this.voice.options.volume !== this.volume) this.volume = this.voice.options.volume;
	}
	...
	public async play(track?: RainlinkTrack, options?: PlayOptions): Promise<RainlinkPlayer> {
		...
		if (!options || typeof options.replaceCurrent !== "boolean") newOptions = { ...options, replaceCurrent: false };

		if (track) {
			if (!newOptions.replaceCurrent && this.queue.current) this.queue.unshift(this.queue.current);
			this.queue.current = track;
		} else this.queue.current ||= this.queue.shift();
		...
		const resolveResult = await current.resolver(this).catch((e: unknown) => {
	 ...
	 * @returns RainlinkPlayer
	 */
	public setLoop(mode: RainlinkLoopMode): this {
		this.checkDestroyed();
		this.loop = mode;
		return this;
	}
	...
	public async seek(position: number): Promise<RainlinkPlayer> {
		...
		if (Number.isNaN(newPosition)) throw new Error("position must be a number");
		if (newPosition < 0 || newPosition > (this.queue.current.duration ?? 0))
			newPosition = Math.max(Math.min(newPosition, this.queue.current.duration ?? 0), 0);
		...
		this.queue.current.position = newPosition;
		return this;
	}
	...
	public async setVolume(volume: number): Promise<RainlinkPlayer> {
		this.checkDestroyed();
		if (Number.isNaN(volume)) throw new Error("volume must be a number");
		await this.node.rest.updatePlayer({
			guildId: this.guildId,
			playerOptions: {
				volume
	 ...
	 * @returns RainlinkPlayer
	 */
	public setMute(enable: boolean): this {
		this.checkDestroyed();
	...
	/**
	 * Stop all activities and reset to default
	 ...
	 * @returns RainlinkPlayer
	 */
	public setDeaf(enable: boolean): this {
		this.checkDestroyed();
